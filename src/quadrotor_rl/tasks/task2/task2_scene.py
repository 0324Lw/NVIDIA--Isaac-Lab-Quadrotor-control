...
def get_quadrotor_task2_asset_source() -> str:
    """Return the latest selected asset source.

    Do not attach this string to InteractiveSceneCfg. Isaac Lab scans scene
    config class attributes and unknown strings may be parsed as assets.
    """

    return _LAST_ASSET_SOURCE


def _try_import_crazyflie_cfg(cfg: Task2Config) -> Tuple[Any | None, str]:
    candidates = [
...
def make_fallback_crazyflie_cfg(cfg: Task2Config) -> ArticulationCfg:
...
def build_quadrotor_asset_cfg(cfg: Task2Config) -> Tuple[Any, str]:
...
def make_quadrotor_task2_scene_cfg(task_cfg: Task2Config | None = None):
    """Build Task2 InteractiveSceneCfg without launching Isaac Sim."""

    global _LAST_ASSET_SOURCE

    if task_cfg is None:
        task_cfg = Task2Config()
    ...
    @configclass
    class QuadrotorTask2SceneCfg(InteractiveSceneCfg):
    ...
    return QuadrotorTask2SceneCfg


QuadrotorTask2SceneCfgFactory = make_quadrotor_task2_scene_cfg
CrazyflieTask2SceneCfgFactory = make_quadrotor_task2_scene_cfg
