...
parser = argparse.ArgumentParser(
    description="Train Quadrotor / Crazyflie Task2 3D Trajectory Tracking with TRUE skrl PPO"
)

# Env / run
parser.add_argument("--num-envs", type=int, default=128)
parser.add_argument("--total-env-steps", type=int, default=150_000_000)
...
parser.add_argument("--entropy-loss-scale", type=float, default=0.010)
parser.add_argument("--value-loss-scale", type=float, default=1.0)
parser.add_argument("--grad-norm-clip", type=float, default=0.5)

# Policy distribution
parser.add_argument("--init-log-std", type=float, default=-0.65)
...
parser.add_argument("--max-dev-err", type=float, default=2.0)
parser.add_argument("--target-search-range", type=int, default=30)
...
def try_partial_load_task1_policy(models: Dict[str, nn.Module], path: str) -> bool:
    """Best-effort warm-start from Task1 eval checkpoint.

    Task1 obs dim is 108 and Task2 obs dim is 100, so full load is usually not
    shape-compatible. This function only copies exactly matching tensors.
    """
    ...
    try:
        ...
        print(f"[WARN] partial task1 load failed to read checkpoint: {type(exc).__name__}: {exc}")
        return False
    ...
    if not isinstance(src_state, dict):
        print("[WARN] partial task1 load skipped: no policy state found")
        return False
    ...
    print(f"[INFO] Task1 partial policy warm-start copied {copied}/{total} tensors")
    return copied > 0
...
def save_training_metadata(
    ...
    metadata = {
        "stage": "quadrotor_task2_3d_trajectory_tracking_skrl",
        "uses_skrl": True,
        "algorithm": "skrl PPO",
        ...
        "lookahead_steps": int(base_env.cfg.lookahead_steps),
        "lookahead_interval": int(base_env.cfg.lookahead_interval),
        "trajectory_num_points": int(base_env.num_points),
...
def save_eval_checkpoint(
    ...
    ckpt = {
        ...
        "metadata": {
            "uses_skrl": True,
            "algorithm": "skrl PPO",
            "task": "quadrotor_task2_3d_trajectory_tracking",
            "num_envs": int(base_env.num_envs),
            "actor_obs_dim": int(env.observation_space.shape[0]),
            "critic_obs_dim": int(env.state_space.shape[0]),
            "action_dim": int(env.action_space.shape[0]),
    ...
    torch.save(ckpt, os.path.join(path, "quadrotor_task2_model.pt"))
...
def save_all_checkpoints(
    ...
    agent.save(os.path.join(save_dir, "quadrotor_task2_skrl_model.pt"))
...
class QuadrotorTask2SkrlWrapper(gym.Env):
    """Adapter from tested Tensor Gym API to skrl IsaacLab dict observations."""

    metadata = {"render_modes": []}

    def __init__(self, env: QuadrotorTask2Env):
        super().__init__()
    ...
    def step(self, actions):
        ...
        obs = torch.nan_to_num(obs, nan=0.0, posinf=10.0, neginf=-10.0)
        state = torch.nan_to_num(state, nan=0.0, posinf=20.0, neginf=-20.0)
        reward = torch.nan_to_num(reward, nan=0.0, posinf=120.0, neginf=-30.0)
...
class QuadrotorTask2Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        self.apply(QuadrotorTask2Actor._orthogonal_init)
...
def build_skrl_cfg(env, log_dir: str) -> Dict[str, Any]:
    ...
    cfg["experiment"].update(
        {
            "directory": log_dir,
            "experiment_name": "quadrotor_task2_3d_trajectory_tracking",
...
def print_update(
    ...
    pbar.write(
        "\n".join(
            [
                "\n" + "=" * 122,
                f"📊 [Quadrotor Task2 PPO 更新 {update_id}] "
                f"步数: {env_steps:,} / {total_steps:,} | "
                f"FPS: {stat['fps_env_steps']:,.0f} | LR: {lr:.3e} | "
                f"Dist: {tel.get('Dist_Error', 0):.3f} | "
                f"Comp: {tel.get('Completion_Rate', 0):.3f} | "
                f"Z: {tel.get('Z', 0):.3f} | "
                f"RP: {tel.get('RollPitchAbs', 0):.3f} | "
                f"Vel: {tel.get('Vel_Align', 0):+.3f} | "
...
def main() -> None:
    ...
    log_root = PROJECT_ROOT / "logs" / "task2"
    run_name = f"quadrotor_task2_skrl_ppo_tracking_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    log_dir = str(log_root / run_name)
    os.makedirs(log_dir, exist_ok=True)

    print("\n" + "=" * 122)
    print("🚁 Quadrotor / Crazyflie Task2: 3D Trajectory Tracking TRUE skrl PPO Training")
    print("=" * 122)
    ...
    env_cfg.max_dev_err = float(args_cli.max_dev_err)
    env_cfg.target_search_range = int(args_cli.target_search_range)
    env_cfg.print_debug_info = bool(args_cli.print_debug_info)

    env_cfg.validate()

    base_env = QuadrotorTask2Env(env_cfg)
    skrl_base_env = QuadrotorTask2SkrlWrapper(base_env)
    env = wrap_env(skrl_base_env, wrapper="isaaclab")

    num_envs = int(getattr(env, "num_envs", skrl_base_env.num_envs))

    print("\n[DEBUG] Quadrotor Task2 skrl spaces")
    ...
    print("\n[INFO] Quadrotor Task2 skrl PPO 训练配置")
    print(f"  - num_envs              : {num_envs:,}")
    ...
    print(f"  - trajectory_num_points : {env_cfg.trajectory_num_points}")
    print(f"  - lookahead             : {env_cfg.lookahead_steps} x {env_cfg.lookahead_interval}")
    print(f"  - action_scale          : {env_cfg.action_scale}")
    print(f"  - action_ema_alpha      : {env_cfg.action_ema_alpha}")
    print(f"  - max_dev_err           : {env_cfg.max_dev_err}")
    print(f"  - target_search_range   : {env_cfg.target_search_range}")
    ...
    if args_cli.resume:
        ...
        task1_partial_loaded = try_partial_load_task1_policy(models, args_cli.pretrained_task1)
        sanitize_agent_numerics(agent, models, args_cli.min_log_std, args_cli.max_log_std)
    ...
    print("\n🔥 [Quadrotor Task2 TRUE skrl PPO 已启动]")
    print("👉 重点观察：Dist_Error / Completion_Rate / Z / RollPitchAbs / Vel_Align / Success_Rate / Crash_Rate / Deviation_Rate")
    print(f"👉 TensorBoard: tensorboard --logdir={PROJECT_ROOT / 'logs'}\n")
    ...
    try:
                ...
                if not info and hasattr(base_env, "episode_steps"):
                    info = {
                        "telemetry": {
                            ...
                            "Target_Idx": base_env.target_idx.float().mean().item(),
                            "Completion_Rate": (base_env.target_idx.float() / max(float(base_env.num_points - 1), 1.0)).mean().item(),
                ...
                pbar.set_postfix(
                    {
                        ...
                        "dist": f"{tel.get('Dist_Error', 0):.3f}",
                        "comp": f"{tel.get('Completion_Rate', 0):.3f}",
                        "z": f"{tel.get('Z', 0):.3f}",
        ...
        print("\n[ERROR] Quadrotor Task2 skrl PPO 训练过程中发生真实异常：")
        traceback.print_exc()
    finally:
        ...
        try:
            ...
            print(f"✅ Quadrotor Task2 skrl 模型与归一化统计已保存至 {final_dir}")
        except Exception as exc:
        ...
        print("✅ Quadrotor Task2 skrl PPO 训练管线安全退出")
