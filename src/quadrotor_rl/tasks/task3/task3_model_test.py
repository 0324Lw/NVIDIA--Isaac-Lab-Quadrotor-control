...
parser = argparse.ArgumentParser(
    description="Evaluate Quadrotor / Crazyflie Task3 TRUE skrl PPO model"
)
...
parser.add_argument(
    ...
    type=str,
    default="fixed_easy",
    choices=["fixed_easy", "fixed_medium", "fixed_hard", "custom"],
)
parser.add_argument("--eval-static", type=int, default=5)
parser.add_argument("--eval-dynamic", type=int, default=0)
parser.add_argument("--eval-max-dist", type=float, default=25.0)
...
def resolve_checkpoint(path: str) -> Path:
    ...
    if p.is_dir():
        candidates = [
            p / "quadrotor_task3_model.pt",
            p / "final_checkpoint" / "quadrotor_task3_model.pt",
            p / "checkpoint_5000" / "quadrotor_task3_model.pt",
        ]
        ...
        for pt in pt_files:
            if pt.name.endswith("_preprocessor.pt"):
                continue
            if pt.name in {"train_metadata.pt", "quadrotor_task3_skrl_model.pt"}:
                continue
            return pt
...
def load_policy_checkpoint(ckpt_path: Path, env: QuadrotorTask3Env):
    ckpt = torch_load_checkpoint(ckpt_path, env.device)

    if not isinstance(ckpt, dict) or "policy" not in ckpt:
        raise RuntimeError(
            f"当前测试脚本需要 task3_train.py 保存的 eval checkpoint: quadrotor_task3_model.pt\n"
    ...
    if "task3" not in task_name and "navigation" not in task_name:
        raise RuntimeError(f"checkpoint task metadata 不是 Task3 dynamic obstacle navigation: {task_name}")
    ...
    expected_lidar_rays = int(metadata.get("lidar_num_rays", env.cfg.lidar_num_rays))
    ...
    if expected_lidar_rays != env.cfg.lidar_num_rays:
        raise RuntimeError(f"lidar rays mismatch: checkpoint={expected_lidar_rays}, env={env.cfg.lidar_num_rays}")
    ...
    policy = QuadrotorTask3Actor(
        ...
        device=env.device,
        init_log_std=float(train_args.get("init_log_std", -0.60)),
...
def eval_curriculum_tuple() -> tuple[str, int, int, float]:
    mode = str(args_cli.eval_curriculum)

    if mode == "fixed_easy":
        return "fixed_easy", 5, 0, 25.0

    if mode == "fixed_medium":
        return "fixed_medium", 10, 2, 25.0

    if mode == "fixed_hard":
        return "fixed_hard", 25, 4, 45.0

    return "custom", int(args_cli.eval_static), int(args_cli.eval_dynamic), float(args_cli.eval_max_dist)


def apply_eval_curriculum(env: QuadrotorTask3Env) -> str:
    label, num_static, num_dynamic, max_dist = eval_curriculum_tuple()
    env.set_curriculum(num_static=num_static, num_dynamic=num_dynamic, max_sg_dist=max_dist)
    env.reset()
    return label
...
def print_summary_table(summary: Dict[str, Dict[str, float]]) -> None:
    print("\n" + "=" * 188)
    print("Quadrotor / Crazyflie Task3 TRUE skrl PPO Model Test Summary")
    print("=" * 188)
    print(
...
def save_tracking_plot(path: str, records: List[Dict[str, float]]) -> None:
    ...
    goal = np.asarray([row.get("telemetry/Goal_Dist", np.nan) for row in records], dtype=np.float32)
    progress = np.asarray([row.get("telemetry/Progress", np.nan) for row in records], dtype=np.float32)
    min_lidar = np.asarray([row.get("telemetry/Min_Lidar", np.nan) for row in records], dtype=np.float32)
    reward = np.asarray([row.get("test/reward_mean", np.nan) for row in records], dtype=np.float32)

    plt.figure(figsize=(12, 5))
    plt.plot(steps, goal, label="Goal distance")
    plt.plot(steps, min_lidar, label="Min lidar distance")
    plt.plot(steps, progress, label="Progress")
    plt.plot(steps, reward, label="Reward mean", alpha=0.7)
    plt.xlabel("Logged evaluation step")
    plt.ylabel("Value")
    plt.title("Task3 Navigation Evaluation")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    ...
    print(f"[OK] navigation plot saved to: {p}")
...
def build_env() -> QuadrotorTask3Env:
    ...
    if bool(args_cli.hold_success):
        cfg.success_radius = 1.0e-6
        cfg.success_xy_tolerance = 1.0e-6
        cfg.success_z_tolerance = 1.0e-6
...
def main() -> None:
    ...
    env = build_env()
    curriculum_label = apply_eval_curriculum(env)
    ...
    print("\n" + "=" * 150)
    print("Quadrotor / Crazyflie Task3 TRUE skrl PPO model test started")
    print("=" * 150)
    ...
    print(f"lidar                   : {env.cfg.lidar_num_rays} rays, range={env.cfg.lidar_max_range}")
    ...
    print(f"eval_curriculum         : {curriculum_label}")
    print(f"active_static_obs       : {env.cfg.num_static_obs}")
    print(f"active_dynamic_obs      : {env.cfg.num_dynamic_obs}")
    print(f"max_start_goal_dist     : {env.cfg.max_start_goal_dist}")
    ...
    try:
        with tqdm(
            ...
            desc="Quadrotor Task3 skrl Model Test",
            dynamic_ncols=True,
            mininterval=0.5,
        ) as pbar:
                ...
                if step % max(int(args_cli.print_interval), 1) == 0 or step == int(args_cli.steps) - 1:
                    ...
                    pbar.set_postfix(
                        {
                            ...
                            "goal": f"{flat.get('telemetry/Goal_Dist', 0.0):.2f}",
                            "prog": f"{flat.get('telemetry/Progress', 0.0):+.3f}",
                            "lidar": f"{flat.get('telemetry/Min_Lidar', 0.0):.2f}",
                            "z": f"{flat.get('telemetry/Pos_Z', 0.0):.2f}",
                            "succ": f"{flat.get('events/Success_Rate', 0.0):.3f}",
                            "coll": f"{flat.get('events/Obstacle_Collision_Rate', 0.0):.3f}",
                    ...
                    if bool(args_cli.visualize):
                        sys.stdout.write(
                            ...
                            f"Nearest={flat.get('telemetry/Nearest_Obstacle', 0.0):.3f} | "
                            f"VelToward={flat.get('telemetry/Vel_Toward', 0.0):+.3f} | "
                            ...
                            f"Obstacle_Collision_Rate={flat.get('events/Obstacle_Collision_Rate', 0.0):.3f} | "
        ...
        print("\n✅ Quadrotor Task3 TRUE skrl PPO model test rollout finished")
        ...
        print("Quadrotor Task3 model test checklist:")
        print("1. checkpoint metadata 必须标记 uses_skrl=True。")
        print("2. actor obs 必须为 300 维。")
        print("3. action 必须为 4 维。")
        print("4. smoke checkpoint 效果差是正常的，先看加载、rollout 和无 NaN/Inf。")
        print("5. 正式效果重点看 Goal_Dist、Progress、Min_Lidar、Heading_Align、Success_Rate、Obstacle_Collision_Rate、Crash_Rate、Deviation_Rate。")
