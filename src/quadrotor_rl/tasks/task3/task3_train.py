...
parser = argparse.ArgumentParser(
    description="Train Quadrotor / Crazyflie Task3 Dynamic Obstacle Navigation with TRUE skrl PPO"
)
...
parser.add_argument("--entropy-loss-scale", type=float, default=0.012)
...
parser.add_argument("--action-ema-alpha", type=float, default=0.50)
parser.add_argument("--safe-lidar-dist", type=float, default=0.25)
parser.add_argument("--success-radius", type=float, default=0.60)
parser.add_argument("--print-debug-info", action="store_true")

# Curriculum
parser.add_argument("--curriculum-mode", type=str, default="serial", choices=["serial", "fixed_easy", "fixed_medium", "fixed_hard"])
parser.add_argument("--phase1-frac", type=float, default=0.34)
parser.add_argument("--phase2-frac", type=float, default=0.67)
parser.add_argument("--phase1-static", type=int, default=5)
parser.add_argument("--phase1-dynamic", type=int, default=0)
parser.add_argument("--phase1-max-dist", type=float, default=25.0)
parser.add_argument("--phase2-static", type=int, default=10)
parser.add_argument("--phase2-dynamic", type=int, default=2)
parser.add_argument("--phase2-max-dist", type=float, default=25.0)
parser.add_argument("--phase3-static", type=int, default=25)
parser.add_argument("--phase3-dynamic", type=int, default=4)
parser.add_argument("--phase3-max-dist", type=float, default=45.0)
...
def try_partial_load_policy(models: Dict[str, nn.Module], path: str, label: str) -> bool:
    """Best-effort warm-start from Task1/Task2 eval checkpoint.

    Task3 obs dim is 300, so full load is usually not shape-compatible.
    This function copies exactly matching tensors only.
    """
...
def current_curriculum_dict(base_env: QuadrotorTask3Env, label: str) -> Dict[str, Any]:
    return {
        "curriculum_label": str(label),
        "num_static_obs_active": int(base_env.cfg.num_static_obs),
        "num_dynamic_obs_active": int(base_env.cfg.num_dynamic_obs),
        "min_start_goal_dist": float(base_env.cfg.min_start_goal_dist),
        "max_start_goal_dist": float(base_env.cfg.max_start_goal_dist),
    }
...
def save_training_metadata(
    ...
    metadata = {
        "stage": "quadrotor_task3_dynamic_obstacle_navigation_skrl",
        "uses_skrl": True,
        "algorithm": "skrl PPO",
        ...
        "lidar_num_rays": int(base_env.cfg.lidar_num_rays),
        "lidar_max_range": float(base_env.cfg.lidar_max_range),
        "static_obs_capacity": int(base_env.world.static_pos.shape[1]),
        "dynamic_obs_capacity": int(base_env.world.dynamic_pos.shape[1]),
        ...
        "curriculum": current_curriculum_dict(base_env, curriculum_label),
        "args": vars(args),
        "env_cfg": cfg_dict,
        "extra": extra or {},
    }
...
def save_eval_checkpoint(
    ...
    ckpt = {
        ...
        "metadata": {
            "uses_skrl": True,
            "algorithm": "skrl PPO",
            "task": "quadrotor_task3_dynamic_obstacle_navigation",
            "num_envs": int(base_env.num_envs),
            "actor_obs_dim": int(env.observation_space.shape[0]),
            "critic_obs_dim": int(env.state_space.shape[0]),
            "action_dim": int(env.action_space.shape[0]),
    ...
    torch.save(ckpt, os.path.join(path, "quadrotor_task3_model.pt"))
...
def save_all_checkpoints(
    ...
    agent.save(os.path.join(save_dir, "quadrotor_task3_skrl_model.pt"))
...
class QuadrotorTask3SkrlWrapper(gym.Env):
    """Adapter from tested Tensor Gym API to skrl IsaacLab dict observations."""

    metadata = {"render_modes": []}

    def __init__(self, env: QuadrotorTask3Env):
        super().__init__()
    ...
    def step(self, actions):
        ...
        obs = torch.nan_to_num(obs, nan=0.0, posinf=10.0, neginf=-10.0)
        state = torch.nan_to_num(state, nan=0.0, posinf=20.0, neginf=-20.0)
        reward = torch.nan_to_num(reward, nan=0.0, posinf=160.0, neginf=-80.0)
...
class QuadrotorTask3Actor(GaussianMixin, Model):
    def __init__(
        self,
        observation_space,
        state_space,
...
class QuadrotorTask3Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        self.apply(QuadrotorTask3Actor._orthogonal_init)
...
def build_skrl_cfg(env, log_dir: str) -> Dict[str, Any]:
    ...
    cfg["experiment"].update(
        {
            "directory": log_dir,
            "experiment_name": "quadrotor_task3_dynamic_obstacle_navigation",
...
def curriculum_for_steps(env_steps: int, total_env_steps: int) -> tuple[str, int, int, float]:
    if args_cli.curriculum_mode == "fixed_easy":
        return "fixed_easy", int(args_cli.phase1_static), int(args_cli.phase1_dynamic), float(args_cli.phase1_max_dist)
    if args_cli.curriculum_mode == "fixed_medium":
        return "fixed_medium", int(args_cli.phase2_static), int(args_cli.phase2_dynamic), float(args_cli.phase2_max_dist)
    if args_cli.curriculum_mode == "fixed_hard":
        return "fixed_hard", int(args_cli.phase3_static), int(args_cli.phase3_dynamic), float(args_cli.phase3_max_dist)

    progress = float(env_steps) / max(float(total_env_steps), 1.0)
    if progress < float(args_cli.phase1_frac):
        return "phase1_easy", int(args_cli.phase1_static), int(args_cli.phase1_dynamic), float(args_cli.phase1_max_dist)
    if progress < float(args_cli.phase2_frac):
        return "phase2_medium", int(args_cli.phase2_static), int(args_cli.phase2_dynamic), float(args_cli.phase2_max_dist)
    return "phase3_hard", int(args_cli.phase3_static), int(args_cli.phase3_dynamic), float(args_cli.phase3_max_dist)


def apply_curriculum_if_needed(base_env: QuadrotorTask3Env, env_steps: int, total_env_steps: int, current_label: str) -> str:
    label, ns, nd, max_dist = curriculum_for_steps(env_steps, total_env_steps)
    if label != current_label:
        print("\n" + "=" * 122)
        print(f"🌍 [Task3 Curriculum Switch] {current_label or '<none>'} -> {label}")
        print(f"   active static={ns}, dynamic={nd}, max_start_goal_dist={max_dist:.2f}")
        print("=" * 122 + "\n")
        base_env.set_curriculum(num_static=ns, num_dynamic=nd, max_sg_dist=max_dist)
        base_env.reset()
    return label
...
def print_update(
    ...
    pbar.write(
        "\n".join(
            [
                "\n" + "=" * 122,
                f"📊 [Quadrotor Task3 PPO 更新 {update_id}] "
                f"步数: {env_steps:,} / {total_steps:,} | "
                f"Stage: {curriculum_label} | "
                f"FPS: {stat['fps_env_steps']:,.0f} | LR: {lr:.3e} | "
                f"Goal: {tel.get('Goal_Dist', 0):.3f} | "
                f"Prog: {tel.get('Progress', 0):+.3f} | "
                f"Z: {tel.get('Pos_Z', 0):.3f} | "
                f"MinLidar: {tel.get('Min_Lidar', 0):.3f} | "
                f"Heading_Align: {tel.get('Heading_Align', 0):+.3f} | "
                f"Succ: {ev.get('Success_Rate', 0):.3f} | "
                f"Crash: {ev.get('Crash_Rate', 0):.3f} | "
                f"Obstacle_Collision_Rate: {ev.get('Obstacle_Collision_Rate', 0):.3f} | "
...
def main() -> None:
    ...
    log_root = PROJECT_ROOT / "logs" / "task3"
    run_name = f"quadrotor_task3_skrl_ppo_navigation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    log_dir = str(log_root / run_name)
    os.makedirs(log_dir, exist_ok=True)

    print("\n" + "=" * 122)
    print("🚁 Quadrotor / Crazyflie Task3: Dynamic Obstacle Navigation TRUE skrl PPO Training")
    print("=" * 122)
    ...
    env_cfg.safe_lidar_dist = float(args_cli.safe_lidar_dist)
    env_cfg.success_radius = float(args_cli.success_radius)
    env_cfg.print_debug_info = bool(args_cli.print_debug_info)

    env_cfg.validate()

    base_env = QuadrotorTask3Env(env_cfg)
    skrl_base_env = QuadrotorTask3SkrlWrapper(base_env)
    ...
    current_curriculum = ""
    current_curriculum = apply_curriculum_if_needed(base_env, start_env_steps, total_env_steps, current_curriculum)

    print("\n[DEBUG] Quadrotor Task3 skrl spaces")
    ...
    print("\n[INFO] Quadrotor Task3 skrl PPO 训练配置")
    print(f"  - num_envs              : {num_envs:,}")
    ...
    print(f"  - lidar                 : {env_cfg.lidar_num_rays} rays, range={env_cfg.lidar_max_range}")
    print(f"  - action_scale          : {env_cfg.action_scale}")
    print(f"  - action_ema_alpha      : {env_cfg.action_ema_alpha}")
    print(f"  - curriculum_mode       : {args_cli.curriculum_mode}")
    print(f"  - initial_curriculum    : {current_curriculum}")
    print(f"  - phase1                : static={args_cli.phase1_static}, dynamic={args_cli.phase1_dynamic}, max_dist={args_cli.phase1_max_dist}")
    print(f"  - phase2                : static={args_cli.phase2_static}, dynamic={args_cli.phase2_dynamic}, max_dist={args_cli.phase2_max_dist}")
    print(f"  - phase3                : static={args_cli.phase3_static}, dynamic={args_cli.phase3_dynamic}, max_dist={args_cli.phase3_max_dist}")
    ...
    print("\n🔥 [Quadrotor Task3 TRUE skrl PPO 已启动]")
    print("👉 重点观察：Goal_Dist / Progress / Min_Lidar / Heading_Align / Success_Rate / Crash_Rate / Deviation_Rate")
    print(f"👉 TensorBoard: tensorboard --logdir={PROJECT_ROOT / 'logs'}\n")
    ...
    try:
        ...
        with tqdm(
            ...
            for t in range(total_vector_steps):
                ...
                current_curriculum = apply_curriculum_if_needed(
                    base_env=base_env,
                    env_steps=absolute_env_steps,
                    total_env_steps=total_env_steps,
                    current_label=current_curriculum,
                )
                ...
                if not info and hasattr(base_env, "episode_steps"):
                    ...
                    info = {
                        "telemetry": {
                            ...
                            "Goal_Dist": base_env.world.distance_to_goal(root_local).mean().item(),
                            "Pos_Z": root_local[:, 2].mean().item(),
                            "Min_Lidar": base_env.current_lidar.min(dim=-1).values.mean().item(),
                ...
                write_scalars(writer, current_curriculum_dict(base_env, current_curriculum), absolute_env_steps, "curriculum")
                ...
                pbar.set_postfix(
                    {
                        ...
                        "stage": current_curriculum,
                        "fps": f"{(absolute_env_steps - start_env_steps) / max(time.time() - start_time, 1e-6):,.0f}",
                        "goal": f"{tel.get('Goal_Dist', 0):.2f}",
                        "prog": f"{tel.get('Progress', 0):+.3f}",
                        "lidar": f"{tel.get('Min_Lidar', 0):.2f}",
                        "succ": f"{ev.get('Success_Rate', 0):.3f}",
                        "crash": f"{ev.get('Crash_Rate', 0):.3f}",
                        "dev": f"{ev.get('Deviation_Rate', 0):.3f}",
                ...
                if (t + 1) % int(cfg["rollouts"]) == 0:
                    ...
                    if update_id % max(int(args_cli.log_interval_updates), 1) == 0:
                        print_update(
                            ...
                            curriculum_label=current_curriculum,
                        )
        ...
        print("\n[ERROR] Quadrotor Task3 skrl PPO 训练过程中发生真实异常：")
        traceback.print_exc()
    finally:
        ...
        try:
            ...
            print(f"✅ Quadrotor Task3 skrl 模型与归一化统计已保存至 {final_dir}")
        except Exception as exc:
        ...
        print("✅ Quadrotor Task3 skrl PPO 训练管线安全退出")
