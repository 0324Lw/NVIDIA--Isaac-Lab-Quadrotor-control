...
def get_quadrotor_task3_asset_source() -> str:
    """Return latest selected Crazyflie asset source.

    This metadata must not be attached to InteractiveSceneCfg because Isaac Lab
    treats unknown scene config attributes as scene assets.
    """

    return _LAST_ASSET_SOURCE


def _try_import_crazyflie_cfg(cfg: Task3Config) -> Tuple[Any | None, str]:
    candidates = [
...
def make_fallback_crazyflie_cfg(cfg: Task3Config) -> ArticulationCfg:
...
def build_quadrotor_asset_cfg(cfg: Task3Config) -> Tuple[Any, str]:
...
def make_quadrotor_task3_scene_cfg(task_cfg: Task3Config | None = None):
    """Build Task3 InteractiveSceneCfg without launching Isaac Sim."""

    global _LAST_ASSET_SOURCE

    if task_cfg is None:
        task_cfg = Task3Config()
    ...
    @configclass
    class QuadrotorTask3SceneCfg(InteractiveSceneCfg):
    ...
    return QuadrotorTask3SceneCfg


QuadrotorTask3SceneCfgFactory = make_quadrotor_task3_scene_cfg
CrazyflieTask3SceneCfgFactory = make_quadrotor_task3_scene_cfg
