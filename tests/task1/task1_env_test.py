...
parser = argparse.ArgumentParser(description="Quadrotor / Crazyflie Task1 hover stabilization environment test")
...
def print_summary(summary: Dict[str, Dict[str, float]]) -> None:
    print("\n" + "=" * 130)
    print("Quadrotor Task1 Env Test Summary")
    print("=" * 130)
...
def check_obs(env: QuadrotorTask1Env, obs: torch.Tensor) -> None:
...
def check_state(env: QuadrotorTask1Env, state: torch.Tensor) -> None:
...
def test_config() -> None:
    ...
    assert cfg.single_actor_obs_dim == 27
    assert cfg.actor_obs_dim == 108
    assert cfg.critic_obs_dim == 108
...
def test_init_reset(env: QuadrotorTask1Env) -> None:
    heading("[测试 2] 环境初始化 / reset / obs / state 检测")
    ...
    assert env.state_space.shape == (108,)
    assert env.action_space.shape == (4,)

    root_local = env._root_pos_w() - env.env_origins
    err = torch.norm(root_local - env.target_pos_local, dim=-1)
...
def test_step_structure(env: QuadrotorTask1Env) -> None:
    ...
    for key in ["R_Pos", "R_Height", "R_Upright", "P_Action_Smooth", "Total"]:
    ...
    for key in ["Pos_Error", "Z", "RollPitchAbs", "Lin_Vel", "Ang_Vel", "Motor_Multiplier"]:
...
def run_action_response(env: QuadrotorTask1Env, action_value: torch.Tensor, steps: int = 40):
    env.reset()
...
def test_control_response(env: QuadrotorTask1Env) -> None:
    heading("[测试 4] 动作控制响应检测：上升 / 下降 / 偏航")
...
def test_wrench_mapping(env: QuadrotorTask1Env) -> None:
    ...
    _, _, torque_b_yaw, _ = env._actions_to_wrench(yaw_cmd)
...
def test_terminal_events(env: QuadrotorTask1Env) -> None:
    heading("[测试 6] 手动触发 crash / deviation / success / timeout 事件检测")

    env.reset()
    env.set_root_pose_for_test(torch.tensor([0.0, 0.0, 0.05], dtype=torch.float32, device=env.device))
    ...
    env.reset()
    env.set_root_pose_for_test(torch.tensor([2.0, 0.0, 1.0], dtype=torch.float32, device=env.device))
    reward, terminated, truncated, info = env.check_events_for_test()
    assert terminated.float().mean().item() > 0.99
    assert info["events"]["Deviation_Rate"] > 0.99
    print_ok(f"xy deviation triggered, Deviation_Rate={info['events']['Deviation_Rate']:.6f}")

    env.reset()
    env.success_counter[:] = int(env.cfg.success_steps_req) - 1
    env.set_root_pose_for_test(torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=env.device))
    reward, terminated, truncated, info = env.check_events_for_test()
    assert terminated.float().mean().item() > 0.99
    ...
    assert terminated.float().mean().item() < 1.0e-6
    print_ok(f"timeout triggered, Timeout_Rate={info['events']['Timeout_Rate']:.6f}")


def random_rollout(env: QuadrotorTask1Env) -> None:
    heading(f"[测试 7] 随机策略 rollout {args_cli.steps} 步，无 NaN / Inf 检测")
...
def run_tests() -> None:
    heading("🚁 Quadrotor / Crazyflie Task1 Hover Stabilization Env 全量测试启动")
    ...
    try:
        ...
        heading("✅ Quadrotor / Crazyflie Task1 环境测试全部通过")

    except Exception as exc:
        print("\n❌ Quadrotor / Crazyflie Task1 环境测试失败：")
